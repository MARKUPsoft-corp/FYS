...
const ProgramsPage: PageComponent = () => {
  ...
  // Sticky subHeader containing the 4-tab bar
  const subHeaderTabs = (
    <div className="sticky top-[calc(var(--sat,0px)+5rem)] z-20 w-full -mx-1 md:-mx-2 lg:-mx-4 w-[calc(100%+0.5rem)] md:w-[calc(100%+1rem)] lg:w-[calc(100%+2rem)] bg-background/90 backdrop-blur-xl border-b border-border/70 shadow-xs transition-all">
      <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 py-2.5 sm:py-3">
        <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto scrollbar-none p-1 sm:p-1.5 rounded-2xl bg-muted/60 border border-border/60 sm:grid sm:grid-cols-4 touch-pan-x">
          {/* Tab 1: Découvrir & Sur-Mesure */}
          <button
            type="button"
            onClick={() => setActiveTab('discover')}
            className={`relative flex items-center justify-center gap-2 py-2 sm:py-2.5 px-3 sm:px-4 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer shrink-0 sm:shrink ${
              activeTab === 'discover'
