...
export function SupplementsTab({
  ...
  const fruitIds = selectedFruits.map((f) => f.id);
  const recommendation = getCocktailRecommendation(fruitIds);
  const recommendedSupplements = LAB_SUPPLEMENTS.filter((s) =>
    recommendation.recommendedIds.includes(s.id),
  );
  const highlighted = LAB_SUPPLEMENTS.find((s) => s.id === recommendation.highlightedSupplementId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative z-20 -mt-5 lg:mt-8 bg-[#28422F] rounded-2xl p-4 border border-primary/20 shadow-lg flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-white/90 text-[12px] md:text-[13px] font-medium leading-relaxed">
            ...
            <span className="text-[#E0982E] font-bold">(cocktail {recommendation.profileLabel})</span>
